...
//    self.messageField.backgroundColor = HBSRandomColor;
    ...
    self.send.textColor = HBSColor(204, 204, 204);
...
- (void)clickSureImage{
    ...
    if ([[self panduan][0]boolValue]) {
        ...
        [manager PUT:@"http://192.168.0.82:10081/user/findpassword" parameters:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
- (void)timerAction{
    ...
    if (self.time < 0) {
        ...
        self.send.text = [NSString stringWithFormat:@"%ld", (long)self.time--];

...
- (IBAction)sureBtn:(UIButton *)sender {
    ...
    if ([[self panduan][0]boolValue]) {
        ...
        [manager PUT:@"http://192.168.0.82:10081/user/1036/mobile" parameters:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
- (void)sendTapAction{
    if ([self phoneProve]) {
        ...
        AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
        manager.securityPolicy.validatesDomainName = YES;
        [manager POST:@"http://192.168.0.82:10081/service/sms" parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
            
        } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            if ([responseObject[@"code"]integerValue] == 200) {
                
              self.sureLabel.text = responseObject[@"result"][@"smscode"];
                HBSLog(@"12346%@", responseObject[@"result"]);
                [self alertWithTitle:@"验证码已经发送到您的手机" message:nil sure:nil cancel:nil];
                
            }else if ([responseObject[@"code"]integerValue] == 400){
                
                [self alertWithTitle:@"手机号未注册" message:nil sure:^{

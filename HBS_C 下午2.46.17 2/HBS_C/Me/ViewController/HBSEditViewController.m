...
@property (nonatomic, strong) NSDictionary *getDic;//获取数据
@end
...
- (void)viewDidLoad {
    ...
    self.getDic = [NSDictionary dictionary];
    [self setUpChildControls];
//    [self getupData];
    
    HBSLog(@"editDic%@", self.editDic);
}
#pragma mark----设置子控件
    ...
    if ([self checkWIFI]) {
        ...
        [HBSNetWork putUrl:[NSString stringWithFormat:@"%@user/1036",HBSNetAdress] parame:params cookie:nil result:^(id result) {
            ...
            if ([result[@"code"]integerValue] == 200) {
                ...
                self.sexLabel.text = result[@"gender"];
                self.nameTextfield.text = result[@"real_name"];
...
- (void)clicktapSexLabel{
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"新郎" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        self.sexLabel.text = self.editDic[@"gender"];
...
//        self.sexLabel.text = self.getDic[@"gender"];

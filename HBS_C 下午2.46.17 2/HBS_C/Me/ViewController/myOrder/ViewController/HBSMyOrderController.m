...
@interface HBSMyOrderController ()<UITableViewDelegate, UITableViewDataSource>
...
- (void)getupData{
    ...
    if ([self checkWIFI]) {
        ...
        [HBSNetWork getUrl:[NSString stringWithFormat:@"%@order?sId=1036&user_type=0&state=200&iPageItem=20&iPageIndex=0", HBSNetAdress] paramaters:params header:nil cookie:nil Result:^(id result) {
            
            if ([result[@"code"]integerValue] == 200) {
                self.orderArr = [HBSOrderModel mj_objectArrayWithKeyValuesArray:result[@"result"]];
                
                self.resultArr = result[@"result"];
                HBSLog(@"我用字典接受的订单%@", self.resultArr);

...
@property (nonatomic, strong) NSString *cancesStr;
...
//付款
@property (nonatomic, strong) NSString *orderAmountStr;//商品价钱
@property (nonatomic, strong) NSString *orderTitleStr;//商品名称
@property (nonatomic, strong) NSString *orderAateStr;//下单时间
@property (nonatomic, strong) NSString *orderSnStr;//订单编号
@property (nonatomic, strong) NSString *orderStoreID;//订单id
...
//    self.serviceTimeLabel.backgroundColor = HBSRandomColor;
    ...
    [self.serviceWaitBtn addTarget:self action:@selector(payClick:) forControlEvents:UIControlEventTouchUpInside];
...
#pragma mark----服务的布局
    ...
    [self.serviceSureOrderBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(self).offset(-15);
        make.right.mas_equalTo(self.serviceFaOrderBtn).offset(0);
...
- (void)setOrderModel:(HBSOrderModel *)orderModel{
    ...
    self.cancesStr = orderModel.order_status;
    ...
    //付款
    self.orderAmountStr = orderModel.order_amount;//商品价钱
    self.orderTitleStr = orderModel.order_title;//商品名称
    self.orderAateStr = orderModel.order_date;//下单时间
    self.orderSnStr = orderModel.order_sn;//订单编号
    self.orderID = orderModel.order_ids;//商品/服务id
       ...
       if ([orderModel.order_type isEqualToString:@"service"]) {
        ...
        self.moneyLabel.text = [NSString stringWithFormat:@"总计: ¥%@", orderModel.order_amount];
           ...
           if ([orderModel.order_status_id isEqualToString:@"0"]) {
           ...
           }else if ([orderModel.order_status_id isEqualToString:@"10"]) {
           ...
           }else if ([orderModel.order_status_id isEqualToString:@"20"] || [orderModel.order_status_id isEqualToString:@"30"]){
           ...
           }else if ([orderModel.order_status_id isEqualToString:@"40"]){
        ...
        if ([orderModel.order_status_id isEqualToString:@"10"]) {
            self.serviceWaitBtn.hidden = NO;
            self.serviceCancesOrderBtn.hidden = NO;
            self.hasCancesOrderBtn.hidden = YES;
        }else{
        ...
        if ([orderModel.order_status_id isEqualToString:@"20"]) {
    ...
    if (self.serviceWaitBtn.hidden == NO) {
...
- (void)serviceCancesOrder:(UIButton *)btn{
    ...
    UIAlertAction *sureAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [HBSNetWork putUrl:[NSString stringWithFormat:@"%@order/%@", HBSNetAdress, params[@"id"]] parame:params header:nil cookie:nil result:^(id result) {
            if ([result[@"code"]integerValue] == 200) {
                ...
                if ([self.delegate respondsToSelector:@selector(serviceCancesOrderBtnDelegate:WithCancesOrder:)]) {
                    [self.delegate serviceCancesOrderBtnDelegate:self WithCancesOrder:btn];
...
- (void)sureCompleteClick:(UIButton *)btn{
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"请在服务者为您完成服务之后进行确认" message:nil preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil];
    UIAlertAction *sureAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [HBSNetWork putUrl:[NSString stringWithFormat:@"%@order/%@", HBSNetAdress, params[@"id"]] parame:params header:nil cookie:nil result:^(id result) {
            if ([result[@"code"]integerValue] == 200) {
                ...
                if ([self.delegate respondsToSelector:@selector(serviceCancesOrderBtnDelegate:WithCancesOrder:)]) {
                    ...
                    [self.delegate serviceSureOrderBtnDelegate:self WithSureOrder:btn];
    ...
    UIAlertAction *sureAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [HBSNetWork putUrl:[NSString stringWithFormat:@"%@order/%@", HBSNetAdress, params[@"id"]] parame:params  header:nil cookie:nil result:^(id result) {
            if ([result[@"code"]integerValue] == 200) {
                ...
                if ([self.delegate respondsToSelector:@selector(goodsSureHasFaHuoBtnDelegate: WithSureGoodsOrder:)]) {
                    
                    [self.delegate goodsSureHasFaHuoBtnDelegate:self WithSureGoodsOrder:btn];
...
- (void)payClick:(UIButton *)btn{
    ...
    self.payDic = @{@"order_sn":self.orderSnStr,@"order_amount":self.orderAmountStr,@"order_date":self.orderAateStr, @"order_title":self.orderTitleStr, @"order_ids":self.orderID}.mutableCopy;

...
//        self.numTF.text = @"1";
        ...
        if ([dic[@"goods_spec"] count] == 0 || [dic[@"goods_spec"] isEqual:[NSNull null]]) {
            _priceLabel.text = [NSString stringWithFormat:@"¥%.2f", [dic[@"goods_price"] floatValue] * [_buyNumber floatValue]];
        }else{
            _priceLabel.text = [NSString stringWithFormat:@"¥%.2f", [dic[@"goods_spec"][_choosePage][@"goods_price"] floatValue] * [_buyNumber floatValue]];
        }
    }
}
- (void)awakeFromNib {

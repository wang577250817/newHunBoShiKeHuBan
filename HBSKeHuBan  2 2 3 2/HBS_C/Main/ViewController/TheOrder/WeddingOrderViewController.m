...
@property (nonatomic, strong)NSMutableDictionary *wzwNote;
...
- (void)viewDidLoad {
    ...
    _wzwNote = [NSMutableDictionary dictionary];
...
{
    [_wzwNote setObject:textField.text forKey:[NSString stringWithFormat:@"%ld", textField.tag]];
    //    UITableViewCell * cell=(UITableViewCell *)[[textField  superview] superview];
    //    NSIndexPath *indexPath=[self.tableView indexPathForCell:cell];
    //    if (indexPath.section==0) {
    //
...
#pragma mark - 留言赋值
    if (_noteTF.text.length == 0) {
        _noteTF.text = [self.wzwNote objectForKey:[NSString stringWithFormat:@"%ld", self.noteTF.tag]]?:@"";
        ...
        if ([self checkWIFI]) {
            ...
            if (_isCar) {
                ...
                for (int i = 0; i < _dataArr.count; i++) {
                    ...
                    NSString *noteStr = [_wzwNote objectForKey:[NSString stringWithFormat:@"%d", i + 2333]];
                    if (noteStr == nil) {
                        noteStr = @"";
                    }
                    [noteDic setObject:noteStr forKey:_dataArr[i][@"store_id"]];

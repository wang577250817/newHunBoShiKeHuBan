...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if([WXApi isWXAppInstalled]) // 判断 用户是否安装微信
    {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getAliOrderPayResult:) name:@"aliPay" object:nil];
    }
}
- (void)dealloc
{
...
- (void)getAliOrderPayResult:(NSNotification *)notification
{
    if ([[[notification.object objectForKey:@"resultStatus"] description] isEqualToString:@"6001"]) {
    ...
    else if ([[[notification.object objectForKey:@"resultStatus"] description] isEqualToString:@"4000"]){
        //支付失败
        [self creatAlertViewOne:@"温馨提示" message:@"支付失败" sureStr:@"确定" sureAction:^(id result) {
            
        }];
    }
    else if ([[[notification.object objectForKey:@"resultStatus"] description] isEqualToString:@"6002"]){
        //网络链接出错
        [self creatAlertViewOne:@"温馨提示" message:@"网络链接出错" sureStr:@"确定" sureAction:^(id result) {
            
        }];
    }
    else if ([[[notification.object objectForKey:@"resultStatus"] description] isEqualToString:@"8000"]){
        //正在处理中
        [self creatAlertViewOne:@"温馨提示" message:@"正在处理中" sureStr:@"确定" sureAction:^(id result) {
            
        }];
    }
    else if ([[[notification.object objectForKey:@"resultStatus"] description] isEqualToString:@"9000"]){
        //订单支付成功

...
{
     //服务质量星星
    _starRatingView = [[TQStarRatingView alloc] initWithFrame:CGRectMake(WIDTH - 210, 310, 200, 30) numberOfStar:kNUMBER_OF_STAR];
//    self.starRatingView.backgroundColor = HBSRandomColor;
    _starRatingView.delegate = self;
    [self.view addSubview:_starRatingView];
    //服务态度星星
    _starRatingView1 = [[TQStarRatingView alloc] initWithFrame:CGRectMake(WIDTH - 210, 360, 200, 30) numberOfStar:kNUMBER_OF_STAR];
    _starRatingView1.delegate = self;
    [self.view addSubview:_starRatingView1];
    //语言沟通星星
    _starRatingView2 = [[TQStarRatingView alloc] initWithFrame:CGRectMake(WIDTH - 210, 410, 200, 30) numberOfStar:kNUMBER_OF_STAR];
...
- (void)publishClick{
    ...
    if ([self checkWIFI]) {
        ...
        [HBSNetWork postUrl:[NSString stringWithFormat:@"%@order/%@/comment",HBSNetAdress,params[@"id"]] parame:params header:nil cookie:nil result:^(id result) {
            
            if ([result[@"code"]integerValue] == 200) {
                
                self.pingDic = result[@"result"];

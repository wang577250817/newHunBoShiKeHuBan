...
@property (nonatomic, strong) NSString *storeGoodRec_id;
        ...
        if ([self checkWIFI]) {
            ...
            [HBSNetWork getUrl:[NSString stringWithFormat:@"%@cart/%@", HBSNetAdress, params[@"id"]] paramaters:params header:nil cookie:nil Result:^(id result) {
                if ([result[@"code"]integerValue] == 200) {
                    ...
                    self.groupArrs = [HBSMyCarHeaderModel mj_objectArrayWithKeyValuesArray:self.carLists];
                    ...
                    backImage.frame = CGRectMake(0, 65, WIDTH, HEIGHT-60);
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self alertWithTitle:@"确定要删除吗?" message:nil sure:^{
        
        HBSMyCarModel *model = self.myCarArr[indexPath.section][indexPath.row];
        self.storeGoodRec_id = model.rec_id;
        
            NSMutableDictionary *params = [NSMutableDictionary dictionary];
            params[@"id"] = [ProjectCache getLoginMessage][@"user_id"];
            params[@"token"] = userToken;
            params[@"rec_ids"] = self.storeGoodRec_id;
            ...
            if ([self checkWIFI]) {
                ...
                [HBSNetWork deleteUrl:[NSString stringWithFormat:@"%@cart/%@?token=%@&rec_ids=%@",HBSNetAdress, params[@"id"], params[@"token"], params[@"rec_ids"]] parame:params cookie:nil result:^(id result) {
                    if ([result[@"code"]integerValue] == 200) {
                        ...
                        // 1.取出对应cell的模型
                        HBSMyCarModel *myCarModel = self.myCarArr[indexPath.section][indexPath.row];
                        // 2.删除数据源模型
                        [self.myCarArr removeObject:myCarModel];
                        HBSLog(@"烦死我了%@ %@", myCarModel, self.storeGoodRec_id);
...
- (void)ShoppingCellDelegate:(HBSMyCarCell *)cell WithSelectButton:(UIButton *)selectBt{
    ...
    headerModel.isSelect =!selectBt.selected;
...
- (void)imputedAllPrice{
    ...
    for (NSArray *goodsArr in self.myCarArr) {
        ...
        NSString *priceText = [NSString stringWithFormat:@"总价 :%d.00元", allPrice];

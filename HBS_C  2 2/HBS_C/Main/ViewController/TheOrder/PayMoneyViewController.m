...
- (void)releaseInfo:(UIButton *)sender
{
    ...
    [HBSNetWork getUrl:[NSString stringWithFormat:@"%@", HBSNetAdress] paramaters:nil header:nil cookie:nil Result:^(id result) {
...
- (void)payNow
{
    ...
    if (self.PayType==0) {
        ...
        [HBSNetWork getUrl:@"" paramaters:dic header:nil cookie:nil Result:^(id result) {
            if ([result[@"code"] integerValue] == 200) {
                NSDictionary *ddPayInfo=@{@"wX_MCH_ID":@"1304509201",@"prepayId":result[@"result"][@"prepay_id"],@"nonceStr":result[@"result"][@"nonce_str"]};
...
-(void)getAlipay:(NSString *)orderString
{
    ...
    [[AlipaySDK defaultService] payOrder:orderString fromScheme:appScheme callback:^(NSDictionary *resultDic) {
        if ([[[resultDic objectForKey:@"resultStatus"] description] isEqualToString:@"6001"]) {
            ...
            //                PayCGViewController *payVC = [[PayCGViewController alloc] init];

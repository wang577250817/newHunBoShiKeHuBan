...
@interface WeddingDetailViewController ()<UITableViewDelegate, UITableViewDataSource, WKNavigationDelegate, UITextFieldDelegate,TypeSeleteDelegete>
{
    ChoseView *choseView;
    UIView *bgview;
    CGPoint center;
...
- (void)crreatWebView
{
    _webView = [[WKWebView alloc]initWithFrame:CGRectMake(0, 0, WIDTH, 100)];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.section) {
        ...
        case 1:
            ...
            break;
        case 2:
            return self.webView.frame.size.height + 40;
...
- (void)sureAction
{
    ...
    if (isCarOrBuy == 0) {
//        NSLog(@"%@", choseView.countView.tf_count.text);
//        NSLog(@"尺码:%d", choseView.sizeView.seletIndex);
        if (isSpec) {
            if (choseView.sizeView.seletIndex == -1) {
                [self creatAlertViewOne:@"请选择规格" message:nil sureStr:@"确定" sureAction:^(id result) {
                }];
            }else{
                 NSDictionary *dic = @{@"id":@"1036",@"token":@"c02b68e88c44385f56064ce4cd49b319",@"goods_id":self.dataDic[@"goods_id"],@"spec_id":self.guigeArr[choseView.sizeView.seletIndex][@"goods_spec_id"],@"quantity":choseView.countView.tf_count.text};
                [self addCar:dic];
            }
        }else{
            NSDictionary *dic = @{@"id":@"1036",@"token":@"c02b68e88c44385f56064ce4cd49b319",@"goods_id":self.dataDic[@"goods_id"],@"quantity":choseView.countView.tf_count.text};
            [self addCar:dic];
        }

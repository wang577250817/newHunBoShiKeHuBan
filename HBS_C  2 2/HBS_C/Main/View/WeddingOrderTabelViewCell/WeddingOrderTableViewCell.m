...
- (void)setUp_UI
{
    ...
    [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake((WIDTH - 120) * WSHIPEI, 30));
    }];
...
//        self.numTF.text = dic[@"goods_quantity"];
        self.numberLabel.text = [NSString stringWithFormat:@"购买数量:%@", dic[@"goods_quantity"]];
        self.specLabel.text = [NSString stringWithFormat:@"{%@}", dic[@"goods_specification"]];
    }else{
//        self.numTF.text = @"1";
        self.numberLabel.text = [NSString stringWithFormat:@"购买数量:%@", self.buyNumber];
